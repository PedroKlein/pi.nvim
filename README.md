...
- A configured API key (e.g. `ANTHROPIC_API_KEY`)
...
```lua
{
  ...
  cmd = { "Pi", "PiSend", "PiQuick", "PiModel", "PiSession" },
  keys = {
...
- **Inline diff**: `refactor`, `fix`, `docs` — shows a diff split; `<leader>aa` to accept, `<leader>ax` to reject
...
require("pi").setup({
  ...
  split = "vertical",
  split_size = nil, -- nil = auto, or number (cols for vertical, rows for horizontal)
